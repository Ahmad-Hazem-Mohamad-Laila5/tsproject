...
export const inngest = new Inngest({ id: "ts-pro" });
...
const sendMonthlyOffers = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    let sentCount = 0;
    //   send in batch of 10 to avoid overwhelmig mail server
    const batchSize = 10;
  ...
  async ({ event, step }) => {
    ...
    const result = await step.run("assign-rider", async () => {
      ...
      const busyOrders = await prisma.order.findMany({
        where: {
          status: { in: ["Assigned", "Paxked", "Out for Delivery"] },
      ...
      if (!availableRider)
        return { skipped: true, reason: "No riders availble" };
